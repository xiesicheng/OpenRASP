...
int openrasp_concat_handler(ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    bool is_op1_tainted_string = openrasp_taint_possible(op1);
    bool is_op2_tainted_string = openrasp_taint_possible(op2);
    NodeSequence ns;
    ...
    {
        openrasp_taint_mark(result, new NodeSequence(ns) TSRMLS_CC);
    }
    ...
    else
    {
        ...
        if (opline->extended_value == ZEND_ASSIGN_OBJ && Z_OBJ_HT_P(object)->get_property_ptr_ptr)
        {
            ...
            { /* NULL means no success in getting PTR */
                bool is_zptr_tainted_string = openrasp_taint_possible(*zptr);
                bool is_value_tainted_string = openrasp_taint_possible(value);
                ...
                {
                    openrasp_taint_mark(*zptr, new NodeSequence(ns_have_get_ptr) TSRMLS_CC);
                }
                if (OPENRASP_RETURN_VALUE_USED(opline))
                {
                    *retval = *zptr;
        ...
        if (!have_get_ptr)
        {
            ...
            if (z)
            {
                ...
                Z_ADDREF_P(z);
                bool is_z_tainted_string = openrasp_taint_possible(z);
                bool is_value_tainted_string = openrasp_taint_possible(value);
                ...
                {
                    openrasp_taint_mark(z, new NodeSequence(ns_no_get_ptr) TSRMLS_CC);
                }
...
static int openrasp_binary_assign_op_helper(int (*binary_op)(zval *result, zval *op1, zval *op2 TSRMLS_DC), ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    bool is_var_ptr_tainted_string = openrasp_taint_possible(*var_ptr);
    NodeSequence ns_var_ptr = openrasp_taint_sequence(*var_ptr);
    bool is_value_tainted_string = openrasp_taint_possible(value);
    ...
    if (Z_TYPE_PP(var_ptr) == IS_OBJECT && Z_OBJ_HANDLER_PP(var_ptr, get) && Z_OBJ_HANDLER_PP(var_ptr, set))
    {
        ...
        Z_ADDREF_P(objval);
        bool is_objval_tainted_string = openrasp_taint_possible(objval);
        NodeSequence ns_obj;
        ...
        {
            openrasp_taint_mark(objval, new NodeSequence(ns_obj) TSRMLS_CC);
        }
    ...
    else
    {
        ...
        {
            openrasp_taint_mark(*var_ptr, new NodeSequence(ns_nonobj) TSRMLS_CC);
        }
    }
...
void openrasp_taint_mark_strings(zval *symbol_table, std::string varsSource TSRMLS_DC, std::function<bool(char *key)> filter)
{
         ...
         zend_hash_move_forward(ht))
    {
        ...
        {
            openrasp_taint_mark_strings(*ele_value, (varsSource + "['" + name + "']") TSRMLS_CC);
        }
        else if (IS_STRING == Z_TYPE_PP(ele_value))
        {
            openrasp_taint_mark(*ele_value, new NodeSequence(Z_STRLEN_PP(ele_value), varsSource, name, true) TSRMLS_CC);
        }
    }
}
...
int openrasp_assign_handler(ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    if (op1 && *op1 != &EG(error_zval) && Z_TYPE_PP(op1) != IS_OBJECT && PZVAL_IS_REF(*op1) && IS_TMP_VAR != OPENRASP_OP2_TYPE(opline))
    {
        ...
        **op1 = *op2;
        ...
        zval_copy_ctor(*op1);
        zval_dtor(&garbage);
        openrasp_taint_mark(*op1, new NodeSequence(openrasp_taint_sequence(op2)) TSRMLS_CC);
    ...
    {
        SEPARATE_ZVAL(&op2);
        openrasp_taint_mark(op2, new NodeSequence(openrasp_taint_sequence(op2)) TSRMLS_CC);
    }
...
int openrasp_assign_ref_handler(ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    {
        //TODO FETCH DIM/OBJ W SEPARATE_ZVAL_TO_MAKE_IS_REF
        return ZEND_USER_OPCODE_DISPATCH;
    }
    openrasp_free_op free_op1 = {0}, free_op2 = {0};
    ...
    else
    {
        ...
        if (value_type == IS_VAR && opline->extended_value == ZEND_RETURNS_NEW)
        {
    ...
    if (variable_type == IS_VAR)
    {
        ...
        {
            zval_ptr_dtor(&free_op1.var);
            // i_zval_ptr_dtor_nogc(free_op1.var ZEND_FILE_LINE_CC TSRMLS_CC);
        }
    }
    if (value_type == IS_VAR)
    {
        ...
        {
            zval_ptr_dtor(&free_op2.var);
            // i_zval_ptr_dtor_nogc(free_op2.var ZEND_FILE_LINE_CC TSRMLS_CC);
        }
    }
    execute_data->opline++;
    return ZEND_USER_OPCODE_CONTINUE;
    ...
    else if (variable_ptr != value_ptr)
    {
        if (!PZVAL_IS_REF(value_ptr))
        {
            ...
            if (Z_REFCOUNT_P(value_ptr) > 0)
            {
                ...
                {
                    openrasp_taint_mark(value_ptr, new NodeSequence(ns) TSRMLS_CC);
                }
            }
...
int openrasp_qm_assign_handler(ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    if (!((zend_uintptr_t)free_op1.var & 1L))
    {
        ...
        if (openrasp_taint_possible(op1))
        {
            zval *result = &OPENRASP_T(OPENRASP_RESULT_VAR(opline)).tmp_var;
            openrasp_taint_mark(result, new NodeSequence(openrasp_taint_sequence(op1)) TSRMLS_CC);
        }
    }
...
int openrasp_send_var_handler(ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    case IS_VAR:
        op1_type = IS_VAR;
#if (PHP_MAJOR_VERSION == 5) && (PHP_MINOR_VERSION >= 5)
        varptr = openrasp_get_zval_ptr_var(OPENRASP_OP1_NODE_PTR(opline), execute_data, &free_op1 TSRMLS_CC);
#else
        varptr = openrasp_get_zval_ptr_var(OPENRASP_OP1_NODE_PTR(opline), execute_data->Ts, &free_op1 TSRMLS_CC);
#endif
        break;
    case IS_CV:
    {
        op1_type = IS_CV;
        varptr = openrasp_get_zval_ptr_cv(OPENRASP_OP1_NODE_PTR(opline), OPENRASP_GET_ZVAL_PTR_CV_2ND_ARG(BP_VAR_R) TSRMLS_CC);
    }
    break;
    }
    NodeSequence ns;
    if (openrasp_taint_possible(varptr))
    {
        ns = openrasp_taint_sequence(varptr);
    }
    ...
    else if (PZVAL_IS_REF(varptr))
    {
        ...
        if (ns.taintedSize())
        {
            openrasp_taint_mark(varptr, new NodeSequence(ns) TSRMLS_CC);
        }
#else
        if (op1_type == IS_CV ||
...
#if (PHP_MAJOR_VERSION == 5) && (PHP_MINOR_VERSION < 6)
    if (op1_type == IS_CV && free_op1.var)
    {
        zval_ptr_dtor(&free_op1.var);
    }
#endif
...
int openrasp_send_ref_handler(ZEND_OPCODE_HANDLER_ARGS)
{
    ...
    case IS_VAR:
        op1_type = IS_VAR;
#if (PHP_MAJOR_VERSION == 5) && (PHP_MINOR_VERSION >= 5)
        varptr_ptr = openrasp_get_zval_ptr_ptr_var(OPENRASP_OP1_NODE_PTR(opline), execute_data, &free_op1 TSRMLS_CC);
#else
        varptr_ptr = openrasp_get_zval_ptr_ptr_var(OPENRASP_OP1_NODE_PTR(opline), execute_data->Ts, &free_op1 TSRMLS_CC);
#endif
        break;
    case IS_CV:
    {
        op1_type = IS_CV;
#if (PHP_MAJOR_VERSION == 5) && (PHP_MINOR_VERSION < 4)
        varptr_ptr = openrasp_get_zval_ptr_ptr_cv(&opline->op1, execute_data->Ts, BP_VAR_W TSRMLS_CC);
#else
        varptr_ptr = openrasp_get_zval_ptr_ptr_cv(opline->op1.var, BP_VAR_W TSRMLS_CC);
#endif
    }
    break;
    }
...
PHP_FUNCTION(taint_dump)
{
    ...
    if (openrasp_taint_possible(arg))
    {
...
void str_unchange_taint(zval *src, zval *dest TSRMLS_DC)
{
    ...
    {
        openrasp_taint_mark(dest, new NodeSequence(openrasp_taint_sequence(src)) TSRMLS_CC);
    }
}

void openrasp_taint_mark(zval *zv, NodeSequence *ptr TSRMLS_DC)
{
    Z_STRVAL_P(zv) = (char *)erealloc(Z_STRVAL_P(zv), Z_STRLEN_P(zv) + 1 + OPENRASP_TAINT_SUFFIX_LENGTH);
